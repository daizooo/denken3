...
**収録範囲は平成18年度（H18）〜令和8年度上期（R8上）の全回、4科目（理論・電力・機械・法規）**
（2026-07-24 改訂・§11参照。旧記述は「H22〜・21回分」だったが、実際に手元にあるキャプチャ資産の
実態に合わせて拡大する）。取り込みは直近回（R8上・R7下…）から遡順で段階実施し、Phase 2b の完了ラインとする。
...
これを2枚に手動で切り分ける運用は、4科目×H18〜R8上（約20回超）で1回あたり18問前後 ×
毎回想定される総枚数を考えると非現実的（手動切り出しだけで数百回の作業になる）。
...
- `KIKAI_SUBJECT`（`src/data/denken3/kikai/index.ts`）は現状 `papers` フィールド自体が無い。
  理論と同様 `papers: PaperDefinition[]` を持たせ、`registry.ts` の `papersForSubject` から
  引けるようにする（コード上は理論の実装をなぞるだけで、資格・科目の分岐は不要＝§7.8の設計どおり）。
- 理論の `_skeleton.ts`（A問題14問=70点 + B問題15〜18・17/18選択=満点100点）は、ユーザーが
  保有する機械H18のキャプチャが**問1〜問18の18問**であることと一致するため、同一構成を
  機械にも踏襲できる可能性が高い。ただし本設計では**断定しない**——年度・科目によって
  出題数・配点が異なるケースがあるため、`_skeleton.ts` を科目非依存（`subjectId` 引数を取る）に
  汎用化した上で、**回ごとに実際のキャプチャと公式配点表で構成を確認してから**スケルトンを
  確定する運用とする（分野別と同様、正答・配点の転記ミスは採点を直接壊すため）。

### 11.4 次のアクション（実装PRで対応）

1. OCR分割位置検出のローカルスクリプトを試作し、機械H18・18問で検証する。
2. `domain/types.ts` の `PaperQuestion` を §11.2-4 の方向へ修正。
3. `PaperImage.tsx` / `CBTRunner.tsx` / `ResultView.tsx` に `answerYPct` によるマスク表示を実装。
4. `_skeleton.ts` を科目非依存に汎用化し、機械用スケルトンを追加。
5. `KIKAI_SUBJECT` に `papers` を追加し、機械H18回のデータ投入を最初の実データ収録とする。
6. `ImportPanel` の年度別モードを、単一画像＋`denken_question_assets` 登録の両方を行うフローに更新。
