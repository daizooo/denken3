...
- **分野別**教材の問題文・解説の収録（現行どおり書籍参照。アプリはメタデータと進捗のみ）。※年度別は公式公表データを収録する（R4参照）
- 解説文の収録（公式公表は問題と正答のみ。解説は書籍・分野別リンクで補う）
...
| 年度別問題の表示形式 | 問題文・図は**公式PDFから問題単位で切り出した画像**、選択肢・正答・配点はメタデータ | 数式・回路図の転記/レンダリング（KaTeX等）を不要にし、転記ミスをゼロにする。CBT本番も静的な問題表示であり体験は損なわれない |
...
├─ public/papers/denken3/riron/
│  └─ r6-1/a01.png … b18.png       # 公式PDFから切り出した問題画像
├─ lib/
│  ├─ supabase.ts                  # 既存
...
- **中断**: 明示的な中断ボタンで `remaining_seconds` を保存して退出。解答は都度autosaveされるため、ブラウザが落ちても失われない。

#### (3) 採点・結果画面

- 総得点（100点換算）・A/B別得点・60点ラインとの差・所要時間。
- 問題別の○×一覧: 自分の解答 vs 正答。問題画像をその場で開いて振り返り可能。
...
- **誤答の分野別連携**: 誤答問題に `sourceQuestionId` があれば、該当する分野別問題の復習を前倒し（due_date を今日に設定）するアクションをワンタップで提供。解説が必要な場合の導線もここ（書籍該当章の参照情報を表示）。

#### (4) 問題データの取り込みワークフロー（運用）

公式公表の問題PDF・正答表から、回ごとに以下を作成する（Claude Codeセッションで半自動化）:

1. 問題PDFを問題単位で画像に切り出し → `public/papers/denken3/riron/{paperId}/` に配置（幅1200px程度・PNG/WebP）。
2. 正答表から `PaperDefinition`（正答・配点・選択問題フラグ）を `src/data/.../papers/{paperId}.ts` に作成。
...
収録範囲は **CBT/直近年度を優先に約10回分**（R4上〜R7上 目安）から始め、必要に応じて遡る。
...
| **2b** | 年度別データ拡充（〜約10回分）・分野別リンク・分野別集計・弱点連携 | 2026/11上旬（年度別開始前） | 中（大半はデータ入力） |
...
- **著作権・公開範囲**: 試験問題・正答は公式に公表されているが、本アプリは**個人利用**（Googleログイン・RLSで本人のみ）の範囲で収録する。リポジトリ・デプロイ先を公開設定にしない運用を前提とする。
- **画像アセットのサイズ**: 1回分約17枚×10回分でも数十MB程度の見込み。WebP圧縮でリポジトリ管理の許容範囲だが、増加が問題になったら Supabase Storage へ移す（`imageUrl` の指す先を変えるだけで移行可能）。
- **過剰設計の抑制**: 解説文の収録・複数ユーザー・科目合格期限の厳密モデル化は明示的に非スコープ。必要になった時点で追補する。

## 10. 未決事項（実装前に確認したいこと）

1. 年度別の収録範囲: 直近約10回分（R4上〜R7上）スタートで良いか。CBT導入前の筆記時代（H年度）まで遡る必要があるか。
2. CBTモードのタイマー挙動: 本番同等なら一時停止は不可だが、生活実態（育児・仕事の割り込み）を踏まえ「中断＝残り時間保存」を許可する設計とした。厳格モード（中断＝放棄扱い）も欲しいか。
3. 計算用メモ: CBT本番では計算用紙が配られる。アプリ側は「紙を用意して解く」前提で良いか（画面内メモ機能は作らない想定）。
4. 休止期間中の復習（Anki側と二重になる期間）のアプリ上の扱い: `no_new`（復習は継続表示）で良いか。
5. ペース分析の「現在ペース」の窓（本書は直近14日）: 週末集中型の学習パターンなら21日窓の方が安定する可能性。
