...
-- review_history はアプリが書き込んでいるが migration に列定義がないため、ここで存在を保証する
ALTER TABLE denken_reviews ADD COLUMN IF NOT EXISTS review_history JSONB NOT NULL DEFAULT '[]';
-- エントリ形式: { "date": "2026-07-22", "status": "A", "duration_seconds": 187 }
-- duration_seconds は §7.6 の解答時間計測（旧エントリには存在しない・後方互換）
```
...
現在は一覧カード上で直接 A/B/C を押す方式のため、「解き始め」の時刻が取れない。以下のフローに変更する:

1. 問題カードに**「問題を見る」ボタン**を追加。押下で計測開始（`startedAt` を保持し、カードが「解答中」状態になる）。
2. A/B/C 押下で計測終了。経過秒数を `review_history` エントリに記録する:
   `{ date, status, duration_seconds }`（既存エントリは `duration_seconds` なし＝計測前データとして扱う。後方互換）。
3. 「問題を見る」を経ずに A/B/C を押した場合（従来操作）は `duration_seconds` なしで記録し、計測を強制しない。
...
DB: `review_history` は JSONB のためスキーマ変更不要。ただし現行マイグレーションに列定義が
ないため、migration 003 に `ADD COLUMN IF NOT EXISTS review_history JSONB NOT NULL DEFAULT '[]'`
を含め、列の存在をコードとマイグレーションの両方で保証する。
...
| Phase | 内容 | 完了目標 | 規模感 |
|---|---|---|---|
| **0** | リファクタリング: `src/data/` 分離・features分割・migration 003（exam_id・review_history保証）。**挙動変更なし** | 2026/8上旬 | 中（機械的移動が主） |
| **1** | 試験日程設定＋ペース分析＋申込リマインド＋FSRS試験日クリップ・休止期間（migration 004）。**分野別の解答時間計測（§7.6「問題を見る」フロー）と総合分析の骨格（§7.7(2)(3)の時間なし版）もここで導入**し、計測データの蓄積を早期に開始する | 2026/8中旬（育休前） | 中〜大 |
| **2a** | CBT模試エンジン: 解答画面・タイマー・問題別時間計測・採点・解説表示・移行判定・セッション保存（migration 005）＋直近2回分（R8上・R7下）のデータ収録 | 2026/10中旬 | 大 |
