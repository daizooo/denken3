...
> **取り込み時の既定**（`lib/assets.ts#defaultAnswerXPct`）：`ASSET_MAP` の各行に `answerXPct` を明示すればそれを採用。無指定なら `sort=0→50`（問題見開き）／`sort>0→0`（解答続き）を既定とする。分野別インポートはこの値を `answer_x_pct` として登録するので、**次回以降の取り込みにもそのまま適用される**。
