...
export function retentionFor(eventDate, examDate) {
    if (!examDate)
...
export function finalCheckDue(eventDate, examDate) {
    if (!examDate)
        return null;
...
function toFSRSCard(review, now) {
    ...
    return {
        ...
        state: (review.fsrs_state ?? State.New),
...
export function clipDueToExam(due, eventDate, examDate) {
    if (!examDate)
        return due;
    const daysToExam = diffDays(eventDate, examDate);
    if (daysToExam <= 0)
        return due; // 試験日当日/経過後はクリップしない
    const interval = diffDays(eventDate, due);
    if (interval <= 0)
        return due;
    let maxInterval = daysToExam;
    if (daysToExam <= 14)
        maxInterval = Math.min(maxInterval, 7);
    else if (daysToExam <= 28)
        maxInterval = Math.min(maxInterval, 14);
...
export function calcFSRS(current, status, eventDate, examDate) {
    if (status === '未着手')
        return {};
    ...
    if (status === 'S')
        return { due_date: finalCheckDue(eDate, examDate), last_reviewed: eDate };
...
export function deriveFromHistory(history, examDate) {
    ...
    return {
        ...
        status: (sorted.length ? sorted[sorted.length - 1].status : '未着手'),
    };
}
...
export function retrievability(review, today) {
    if (!review || (review.repetitions ?? 0) <= 0)
        return null;
    if (!review.due_date)
        return null;
...
export function defaultReview(questionId) {
    return {
