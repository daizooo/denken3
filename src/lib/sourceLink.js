 ...
 */
export function sourceFrequency(title) {
 ...
 */
export function resolveSource(token) {
    ...
    if (!number)
        return { paperId, questionId: null };
 ...
 */
export function buildSourceLinks(chapters, papers) {
    // 収録済みの回 → その回の問題IDの集合。未収録の回を指す出典と、誤った問番号を区別する。
    const paperQuestionIds = new Map();
 ...
 */
export function partCountFromTitle(title) {
    for (const token of parseSourceTokens(title)) {
        ...
        if (number !== null && number > MAX_SECTION_A)
            return 2;
    }
    return 1;
}
