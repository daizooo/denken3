...
const BANDS = [1, 2, 3];
const MODE_KEYS = ['calc', 'memory', 'unset'];
...
export function buildTimeStats(chapters, reviews) {
    const raw = [];
    for (const c of chapters) {
        for (const q of c.questions) {
            const seconds = latestDurationSeconds(reviews[q.id]);
            // 旧データには30分上限で記録されたものがある。まず現在の絶対上限で足切りする。
            if (seconds === undefined || seconds > MAX_DURATION_SECONDS)
                continue;
...
export function estimateSeconds(q, review, stats) {
...
export function estimateMinutes(q, review, stats) {
    return estimateSeconds(q, review, stats) / 60;
}
// 問題集合の推定所要分の合計。
export function sumEstimateMinutes(questions, reviews, stats) {
...
export function planByBudget(questions, reviews, stats, budgetMinutes) {
    let count = 0;
    let minutes = 0;
    let totalMinutes = 0;
    let filling = true;
...
export function valueDensity(valueScore, minutes) {
