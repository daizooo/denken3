 ...
 */
function impactOf(status, r, attemptsPerMastery) {
 ...
 */
export function forwardSlotsToday(policy, startedToday) {
...
export function planToday(params) {
    ...
    scored.sort((a, b) => {
        if (a.item.density !== b.item.density)
            return b.item.density - a.item.density;
    ...
    //      翌日以降に戻す ―― 総量は減っていない。
    const floor = Math.min(budgetMinutes === null || budgetMinutes <= 0 ? policy.dailyFloor : 1, ordered.length);
    for (const i of ordered) {
    ...
    for (const i of ordered) {
        if (!i.core || selected.has(i.id))
            continue;
        if (minutes + i.minutes > targetMinutes)
            continue;
        take(i);
    }
    for (const i of ordered) {
