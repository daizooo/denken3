...
export function isEstimateValidated(sessions) {
    const finished = sessions.filter(s => s.status === 'finished' && s.mode === 'cbt' && s.score != null);
    return finished.length >= MIN_VALIDATION_SESSIONS;
}
// 目標到達に必要な最小の問題集合を求める。
// est は analytics.estimateScore の結果（出題比率 weight の唯一の出所）。
export function planPassTarget(chapters, reviews, est, marginPoints = DEFAULT_PASS_MARGIN) {
