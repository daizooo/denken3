...
export function elapsedSeconds(t, today, now = Date.now(), capSeconds = MAX_DURATION_SECONDS) {
    if (t.day !== today)
        return undefined; // 「解答中」のまま日をまたいだら破棄
    const totalMs = t.accumulatedMs + (t.startedAt != null ? now - t.startedAt : 0);
    const sec = Math.round(totalMs / 1000);
    if (sec <= 0)
        return undefined;
    if (sec > capSeconds)
        return undefined; // 押し忘れ・中断・放置の外れ値は除外
    return sec;
}
