...
export function dateAtUTCNoon(dateStr) {
...
export function diffDays(from, to) {
    return Math.round((dateAtUTCNoon(toDateStr(to)).getTime() - dateAtUTCNoon(toDateStr(from)).getTime()) / 86400000);
}
...
// 次回復習日の緊急度に応じた文字色
export function dueColorClass(dateStr) {
    const label = formatDue(dateStr);
    if (label.includes('遅延'))
        return 'text-red-500';
    if (label === '今日')
