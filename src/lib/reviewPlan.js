...
// 未着手だけが復習対象外（重み未設定）。
const STATUS_WEIGHT = { C: 1.0, B: 0.7, A: 0.45, S: 0.25 };
...
function bandOf(r, retention) {
    if (r === null)
        return 'low';
    if (r < retention - BAND_HIGH_MARGIN)
        return 'high';
    if (r < retention)
        return 'mid';
    return 'low';
...
export function reviewValue(question, review, today, examDate) {
    ...
    if (statusW === undefined)
        return { score: 0, r: null, risk: 0, band: 'low', frequency };
...
function catchUpDays(daysToExam) {
    if (daysToExam === null)
        return 3;
    if (daysToExam <= 14)
        return 1;
...
// - どちらも due 総数を超えない。
export function planDailyReviews(candidates, today, examDate) {
    const dueCount = candidates.length;
    if (dueCount === 0)
        return { dueCount: 0, urgentCount: 0, recommendedCount: 0 };
    let urgentCount = 0;
    for (const c of candidates) {
        if (reviewValue(c.question, c.review, today, examDate).band === 'high')
            urgentCount++;
    }
